...
func _update_ui():
	if(state.is_player):
		...
		game.mgmt.ui.update_casttime(skills.current_casttime / skills.current_spell.casttime() if skills.current_spell else 0)
	...
	if(name == settings.get_setting("dev", "debug_target")):
		if(state.is_player):
			...
			var debug_output: String = str(stepify(know.values[planner.value.pain], 0.1)) + "; " + str(stepify(know.values[planner.value.focus], 0.1)) + "; " + str(know.flags)
