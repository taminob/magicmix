...
static func precondition() -> ai_planner.knowledge:
	...
	return ai_planner.knowledge.new(spell.self_pain() + spell.self_pain_per_second() * spell.duration(), -(spell.self_focus() + spell.self_focus_per_second() * spell.duration()), 0, 0, ai_planner.knowledge_mask.enemy_in_near)

static func postcondition() -> ai_planner.knowledge:
	var spell = skill_data.spells[spell_id()]
	return ai_planner.knowledge.new(spell.self_pain() + spell.self_pain_per_second() * spell.duration(), spell.self_focus() + spell.self_focus_per_second() * spell.duration())
