...
func goals(pawn: KinematicBody) -> Array:
	...
	if(!pawn.stats.undead && !pawn.stats.dead):
		var survive_goal: ai_planner.goal = ai_planner.goal.new(ai_planner.knowledge.new(), ai_planner.knowledge_mask.pain)
		survive_goal.requirements.values[ai_planner.value.pain] = max(pawn.stats.pain - 10, 5) # todo? better requirements
		goals.push_back(survive_goal)
	var fight_goal: ai_planner.goal
	if(pawn.stats.undead || pawn.stats.dead):
		fight_goal = ai_planner.goal.new(ai_planner.knowledge.new(), ai_planner.knowledge_mask.enemy_damaged)
	else:
		fight_goal = ai_planner.goal.new(ai_planner.knowledge.new(), ai_planner.knowledge_mask.pain | ai_planner.knowledge_mask.enemy_damaged)
	...
	var talk_goal: ai_planner.goal = ai_planner.goal.new(ai_planner.knowledge.new(), ai_planner.knowledge_mask.talking)
	talk_goal.requirements.flags[ai_planner.flag.talking] = true
