...
func _ready():
	...
	errors.debug_assert(game.world.boxes[game.levels.current_level_id].has(name), "content of box not defined")
...
func open_box(interactor: character):
	...
	is_open = true
	var item_list = game.world.boxes[game.levels.current_level_id].get(name, [])
	if(item_list.empty()):
		return
