...
void vertex()
{
	VERTEX.y += abs(cos(VERTEX.x * grass_amount) * sin(VERTEX.z * grass_amount)) * grass_height;
}

void fragment()
{
	vec2 wind =  vec2(cos(TIME) + 1., 0.) * 0.005;
	ALBEDO = base_color * textureLod(color_noise, UV * color_noise_scale + wind, color_noise_lod).xyz;
	//ALPHA = 1.;
}
